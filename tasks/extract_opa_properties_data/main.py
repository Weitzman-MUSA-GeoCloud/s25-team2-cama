...
def extract_opa_properties(request):
    print('Extracting Philadelphia OPA Properties data...')
    extract_data(
        'https://opendata-downloads.s3.amazonaws.com/opa_properties_public.csv',
        DIRNAME / 'phl_opa_properties.csv',
        'phl_opa_properties/phl_opa_properties.csv',
    )
    return f'✅ Downloaded to {DIRNAME / "phl_opa_properties.csv"} and uploaded to gs://{BUCKET_NAME}/raw/phl_opa_properties/phl_opa_properties.csv'